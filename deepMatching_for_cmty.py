...
def obtain_cmty_feature_array(G,SG,cmty_list,throd_value):
	...
	return rest_small_cmty,new_cmty_list,feature
...
def repeated_eavalute_accuracy_by_feature(G,sample_rate = 0.8,throd_value = 0.75,limit_cmty_nodes = 10,method = euclidean_metric):
	#print "sample rate: %.4f" % sample_rate
	#print "limit cmty nodes: %d" % limit_cmty_nodes
	G1,G2 = bi_sample_graph(G,sample_rate)

	SG1,SG2,SG1_ret_list,SG2_ret_list = community_detect_graph(G1,G2,detect_method = cnm.community_cnm)

	SG1_rest_small_cmty,SG1_new_cmty_list,SG1_features_list = obtain_cmty_feature_array(G1,SG1,SG1_ret_list,limit_cmty_nodes)
	SG2_rest_small_cmty,SG2_new_cmty_list,SG2_features_list = obtain_cmty_feature_array(G2,SG2,SG2_ret_list,limit_cmty_nodes)
	score_list = obtain_score_between_features(SG1_features_list,SG2_features_list,method = method)

	rate,matched_index,left_cmty_list,right_cmty_list = calculate_accuracy_rate_by_feature(G1,SG1_rest_small_cmty,SG1_new_cmty_list,G2,SG2_rest_small_cmty,SG2_new_cmty_list,score_list,SG1_features_list,SG2_features_list)

	return rate,SG1_ret_list,SG2_ret_list,SG1_rest_small_cmty,SG2_rest_small_cmty,left_cmty_list,right_cmty_list,matched_index


def calculate_accuracy_rate_by_feature(SG1,SG1_rest_small_cmty,SG1_new_cmty,SG2,SG2_rest_small_cmty,SG2_new_cmty,score_list,SG1_feature,SG2_feature,throd_value = 0.75):
	'''	
	...
	big_new_cmty = SG1_new_cmty if len(SG1_new_cmty) >= len(SG2_new_cmty) else SG2_new_cmty
	small_new_cmty = SG1_new_cmty if len(SG1_new_cmty) < len(SG2_new_cmty) else SG2_new_cmty
	...
	print "the rest nodes in the community"
	temp_rate = calculate_common_nodes_between_cmties(SG1_rest_small_cmty,SG2_rest_small_cmty) 
	print "rate: %.4f" % temp_rate
	if temp_rate >= throd_value:
		print "the rest nodes mapping successful!"
		matched_count += 1
	else:
		print "mapping failed"
		unmatched_count += 1
	...
	return accuracy_rate,matched_index,big_new_cmty,small_new_cmty
...
def main():
	...
	#nx_G = nx.connected_caveman_graph(10, 40)
	#nx_G = nx.gaussian_random_partition_graph(500,24,70,.25,.01)
	#nx_G = nx.random_partition_graph([120,140,100,89,230,145],0.3,0.1)
	#nx_G = nx.read_gml(sys.argv[1])
	...
	#save_fd = os.dup(1)
	#os.dup2(df.fileno(),1)
	#G1,G2 = bi_sample_graph(nx_G,sample_rate = float(sys.argv[2]))

	#SG1_ret_list,SG2_ret_list = sample_detect_graph(nx_G,detect_method = graph_spectral.partition_super_cliques)
	#SG1_ret_list,SG2_ret_list = community_detect_graph(G1,G2,detect_method = cnm.community_cnm)
	#SG1_ret_list,SG2_ret_list = community_detect_graph(G1,G2,detect_method = cnm.community_cnm_with_limit)

	#obtain the feature SG1
	#SG1_rest_small_cmty,SG1_new_cmty_list,SG1_features_list = obtain_cmty_feature_array(G1,SG1_ret_list,throd_value)
	#SG2_rest_small_cmty,SG2_new_cmty_list,SG2_features_list = obtain_cmty_feature_array(G2,SG2_ret_list,throd_value)
	#score_list = obtain_score_between_features(SG1_features_list,SG2_features_list)
	sum_acc = 0.0
	rate_list = []
	for i in range(repeated_count):
		print "->%d"%i ,
		sys.stdout.flush()
		rate,SG1_ret_list,SG2_ret_list,SG1_rest_small_cmty,SG2_rest_small_cmty,SG1_new_cmty_list,SG1_new_cmty,matched_index = repeated_eavalute_accuracy_by_feature(nx_G,float(sys.argv[2]),limit_cmty_nodes = throd_value)
		df.write("%ith loop:\n"%i)
		df.write("G1 cmty length distribution: ")
		for index in SG1_ret_list:
			df.write("%i " % (len(index)))
			df.flush()
		df.write("\n")
		df.write("G2 cmty length distribution: ")
		for index in SG2_ret_list:
			df.write("%i " % (len(index)))
			df.flush()
		df.write("\n")
	...
	df.write("########################################################################\n")
	df.flush()
	#calculate_accuracy_rate_by_feature(G1,SG1_rest_small_cmty,SG1_new_cmty_list,G2,SG2_rest_small_cmty,SG2_new_cmty_list,score_list,SG1_features_list,SG2_features_list)
	#calculate_accuracy_rate(SG1_ret_list,SG2_ret_list,SG1_max_degree_list ,SG2_max_degree_list,score)
	df.close();
	#os.dup2(save_fd,1)
